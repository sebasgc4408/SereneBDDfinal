    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-[24px] shadow-[0_4px_24px_rgb(0,0,0,0.02)] border border-[#F2F2F0] p-5 md:p-6 flex flex-col gap-5">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-[15px] font-medium text-[#292524] truncate">
              Google Calendar
            </h3>
            <div className="flex items-center gap-1.5 text-[13px] mt-0.5">
              ...
              />
              <span className={calendarConnected ? 'text-[#788B80]' : 'text-[#A8A29E]'}>
                {calendarConnected ? 'Active & Synced' : 'Not Connected'}
              </span>
            </div>
          </div>
        </div>
        ...
        {calendarConnected && onDisconnectCalendar && (
          <button
            onClick={() => void onDisconnectCalendar()}
            className="w-full py-2.5 rounded-xl border border-[#F2F2F0] bg-white text-[13px] font-medium text-[#D9534F] hover:bg-[#FEF5F5] hover:border-[#F5C2C2] transition-colors"
          >
            Disconnect Calendar
          </button>
        )}
      ...
      <div className="bg-[#FAFAF9] rounded-[24px] border border-[#F2F2F0] overflow-hidden">
        <button
          ...
          className="w-full px-5 py-4 flex items-center justify-between text-[13px] font-medium text-[#57534E] hover:bg-[#F2F4F3] transition-colors"
        >
        ...
        <div
          className={`overflow-hidden transition-all duration-300 ease-in-out px-5 ${
            isClarityOpen ? 'max-h-48 opacity-100 pb-5' : 'max-h-0 opacity-0 pb-0'
          }`}
        >
          <div className="space-y-3 pt-1">
            <p className="text-[12px] text-[#78716C] leading-relaxed">
              We only read busy blocks to avoid double booking and never store private event details.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[24px] shadow-[0_4px_24px_rgb(0,0,0,0.02)] border border-[#F2F2F0] p-5 md:p-6 flex flex-col gap-5">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 text-[13px] mt-0.5">
              <span
                className={`h-1.5 w-1.5 rounded-full ${
                  whatsappConnected ? 'bg-[#25D366]' : 'bg-[#A8A29E]'
                }`}
              />
              <span className={whatsappConnected ? 'text-[#788B80]' : 'text-[#A8A29E]'}>
        ...
        {whatsappConnected && onDisconnectWhatsApp && (
          <button
      ...
      <div className="bg-[#FAFAF9] rounded-[24px] border border-[#F2F2F0] overflow-hidden">
        ...
        >
          ...
          <svg
            className={`w-4 h-4 text-[#A8A29E] transition-transform duration-300 ${
              isWhatsAppClarityOpen ? 'rotate-180' : ''
            }`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
        ...
        <div
          className={`overflow-hidden transition-all duration-300 ease-in-out px-5 ${
            isWhatsAppClarityOpen ? 'max-h-48 opacity-100 pb-5' : 'max-h-0 opacity-0 pb-0'
          }`}
        >
          <div className="space-y-3 pt-1">
            <p className="text-[12px] text-[#78716C] leading-relaxed">
              We only send clinical reminders and confirmations. No promotional campaigns.
            </p>
          </div>
        </div>
      </div>
